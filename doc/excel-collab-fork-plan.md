...
| `Aims-Innovation/excel-collab` on branch `document-editor` | **Six patches landed** (#1-#5 + #7 revision). `lib/` rebuilt. Tagged `release/document-editor-v0.1.13.3` then `release/document-editor-v0.1.13.4` with the guard fix. Version is `0.1.13-fork.4`. v0.1.13.4 is the recommended pin for web-consumer — the earlier .3 mask rendering failures on live SignalR. |
...
|---|---|---|---|
| 1 | ~~**Guard Worker.postMessage against DataCloneError.** Wrap the canvas worker's postMessage in the fork source so a function-bearing payload doesn't kill the render. Originally the "option A" runtime guard in web-consumer; cleaner at source.~~ **Landed v0.1.13.3 (revised in v0.1.13.4)**: new `src/controller/workerGuard.ts` at `initController`. v0.1.13.3's sanitize-and-retry masked real render bugs by stripping callbacks and silently drawing nothing. **v0.1.13.4** replaces that behavior: the guard is diagnostic-only — on `DataCloneError` it walks the payload, logs the first non-cloneable value's key path (e.g. `payload.autoFilter.onReady`), then rethrows so the caller sees the failure. **Root-cause companion fix in `src/canvas/MainCanvas.ts`**: the four payload fields sourced from `Y.Map#toJSON` (`sheetData`, `customHeight`, `customWidth`, `autoFilter[sheetId]`) are now JSON-round-tripped before posting, stripping any Y.Doc reference that leaked through `toJSON`'s non-Y passthrough (the symptom was yjs's Doc `load` listener showing up in the payload). | `src/controller/workerGuard.ts`, `src/canvas/MainCanvas.ts` | High — catches a class of bugs going forward |
| 7 | **Fix Worker.postMessage guard — stop dropping callbacks, pre-normalize Y-backed payload entries.** Surfaces in `web-consumer` on branch `AzharGhani/consume-excel-collab-fork` against the live SignalR spreadsheet hub. A non-empty `ReceiveYjsBatch` → `Y.applyUpdate` inside `ydoc.transact()` caused a Y.Doc reference to leak through `Y.Map#toJSON`'s non-Y passthrough into the render payload; structured-clone walked it and tripped on yjs's `load` listener. v0.1.13.3's guard sanitized-and-retried with a gutted payload, so the grid silently stayed blank. QA didn't repro because QA has no SignalR backend deployed → no transaction updates → no leak. | See patch #1 (merged into it) | **Landed in v0.1.13.4** |
...
| 5 | ~~**StrictMode-safe TB bootstrap.** `TB()` function (container bootstrap) has an empty-deps `useEffect` that races under React StrictMode — add a per-mount useRef guard.~~ **Landed**. Source-level equivalents of the minified `TB()` are three empty-deps effects: (a) `Excel` root in `src/editor.tsx` (creates Worker + controller), (b) `useCollaboration` init in `src/containers/index.tsx` (fetches doc from provider, addFirstSheet), (c) `useCollaboration` awareness wiring in the same file (registers `awareness.on`/`doc.on`/`controller.on` listeners). All three now gated by a `useRef(false)` flag that flips on first run — second invocation under StrictMode no-ops. Without (c) in particular, each listener was being registered twice causing duplicate range-sync emissions. | `src/editor.tsx`, `src/containers/index.tsx` | Low-medium (latent; may or may not repro on current consumer config) |
...
| 8 | ~~**Hide menubar right-side widgets + permission-gate exports.** Host-app already provides its own user / language / theme controls; the library's User / I18N / Theme widgets are noise in the consumer. Export should be hidden when the user lacks edit permissions (view-only sessions shouldn't be able to download).~~ **Landed v0.1.13.17** as props `hideUser`, `hideI18n`, `hideTheme`, `canExport` on `EditorProps` (and therefore `<Excel>`). All three `hide*` flags default to `false` (upstream demo unchanged); `canExport` defaults to `true`. When `canExport` is `false`, all three Export menu items (XLSX / CSV / JSON) are removed from the File menu. Web-consumer should pass `<Excel hideUser hideI18n hideTheme canExport={hasEditPermission} … />`. | `src/containers/index.tsx`, `src/containers/MenuBar/index.tsx` | Medium |
...
- Fork latest tag: `release/document-editor-v0.1.13.4` (previous: `release/document-editor-v0.1.13.3`, `release/document-editor-v0.1.13.2`)
